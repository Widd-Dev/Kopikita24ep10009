  ...
  const handleCheckout = async () => {
    try {
      ...
      if (checkoutUrl) {
        ...
        toast.error("Gagal membuat checkout. Silakan coba lagi.");
      }
    } catch (error) {
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full">
        <SheetHeader className="flex-shrink-0">
          ...
          <SheetDescription>
            {totalItems === 0 ? "Keranjang Anda kosong" : `${totalItems} item di keranjang`}
          </SheetDescription>
        </SheetHeader>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 bg-secondary/30 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-semibold text-sm mt-1 text-accent">
                          {item.price.currencyCode} {parseFloat(item.price.amount).toLocaleString('id-ID')}
                        </p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t bg-background">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-accent">
                    {items[0]?.price.currencyCode || 'IDR'} {totalPrice.toLocaleString('id-ID')}
                  </span>
                </div>
